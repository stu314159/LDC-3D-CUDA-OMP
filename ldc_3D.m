...
if ((run_dec ~= 'n') && (run_dec ~= 'N'))
    ...
    %run_cmd=sprintf('mpirun -np %d clbm_ldc3D_cudaMPI',Num_proc);
    run_cmd=sprintf('./clbm_ldc3D');
   ...
   % system('export OMP_NUM_THREADS=1');
    ...
    if(plot_output)
        ...
        for i = 0:(Num_proc-1)
            dat_file_name=sprintf('output_density%d.lbm',i);
            fIn_tmp=load(dat_file_name,'-ascii');
            if(i==0)
                fIn = fIn_tmp;
            else
                fIn = [fIn; fIn_tmp];
